...
class KMNode(object):
    def __init__(self, id, exception=0, match=None, visit=False):    # 11111
        self.id = id
...
class KuhnMunkres(object):
    ...
    def set_matrix(self, x_y_values):
        ...
        self.matrix = np.full((self.x_length, self.y_length), -float('inf'))  # 111111
    ...
    def negate_matrix(self):
        self.matrix = [[-value for value in row] for row in self.matrix]
...
def run_kuhn_munkres(x_y_values):
    min_weight=0
    for x,y,weight in x_y_values:
        min_weight=min(min_weight,weight)
    if min_weight<0:
        min_weight=abs(min_weight)+2
        for i in range(len(x_y_values)):
            x_y_values[i]=(x_y_values[i][0],x_y_values[i][1],round(x_y_values[i][2]+min_weight,2))
    process = KuhnMunkres()
    process.set_matrix(x_y_values)
    process.negate_matrix()
    process.km()
